...
interface UseDynamicAnalyticsOptions extends DynamicAnalyticsCoreOptions {
 ...
 *
 *   useEffect(() => {
 *     // initialize() loads the config from a URL - the primary way to initialize in production.
 *     // initializeWithConfig() is also available, but is intended for unit tests / pre-deployment
 *     // validation of an in-memory config only.
 ...
 * ```
 */
declare function useDynamicAnalytics(options?: UseDynamicAnalyticsOptions): DynamicAnalyticsCore;

export { type UseDynamicAnalyticsOptions, useDynamicAnalytics };
